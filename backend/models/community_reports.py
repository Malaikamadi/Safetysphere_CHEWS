...
def _load_model():
    ...
    if joblib is None or not MODEL_PATH.exists():
        print(f"[CHEWS] Community model or joblib not found. Using fallback heuristic.")
        return False
    ...
    try:
        ...
        print(f"[CHEWS] Community reports model loaded — {type(_model).__name__}")
        return True
    except Exception as e:
        print(f"[CHEWS] Failed to load community model: {e}. Using fallback.")
        return False
